...
<#
    ...
    Version 1.04 [Author: Larry Song; Time: 2015-01-05]
        Exit function when list name is empty.
#>
...
'Alias', 'OU', $KeyProperty, 'AdvanceOptions', 'Modified By', 'Modified', 'Created By', 'Created' | %{
    $objRaw | Add-Member -MemberType NoteProperty -Name $_ -Value $null -Force
}

$JobsLeft = @()
$objJob = New-Object PSObject -Property @{ID = $null; User = $null; VDIName = $null; Type = $null; Exception = $null; CreatedBy = $null}
    ...
    if($Item[$KeyProperty]){
        ...
        $strAlias = $strAlias.Split(',') | %{if($_){$_.Trim()}} | %{
            if($_ -imatch $VDI_AliasFormat){$_}else{
                ...
                $JobsLeft[-1].'User' = $_
                $JobsLeft[-1].'Exception' = 'Invalid alias'
        ...
        $strAlias | %{
            if(!(Get-ADExisting -SAMAccountName $_ -Type user)){
                ...
                $JobsLeft += $objJob.PSObject.Copy()
                $JobsLeft[-1].'User' = $_
                $JobsLeft[-1].'Exception' = 'User not found in AD'
                $JobsLeft[-1].'CreatedBy' = $Item['Created By'] -ireplace '.*?#', ''
                return
            }
            ...
            $RawData[-1].'OU' = $Item['OU']
            $RawData[-1].'AdvanceOptions' = $Item['AdvanceOptions']
            ...
            $VDI_Prefix | %{
                Get-AvailableVDIName -User $RawData[-1].'Alias' -Prefix $_ | Set-Content -Path "$LocalDes\$strDate\Exchanges\$($RawData[-1].'Alias')_${_}_Available.txt"
            }
        }
    }else{
        $JobsLeft += $objJob.PSObject.Copy()
        $JobsLeft[-1].'User' = $Item['Alias']
